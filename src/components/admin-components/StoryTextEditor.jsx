...
const toggleMark = (editor, format, constituentValue) => {
  ...
  !!Object.keys(Editor.marks(editor)).length
    ...
    : Transforms.setNodes(
        editor,
        { [format]: constituentValue || true },
        { at: selectionRange, match: Text.isText, split: true }
      )
}
...
const FormattingToolbar = ({ constituentValue, setConstituentValue }) => {
  ...
  const MarkButton = ({
    ...
    return (
      <Box display='flex' flexDirection='column' alignItems='center'>
        <ToggleButton
          value={format}
          selected={Editor.marks(editor) && !!Editor.marks(editor)[format]}
          sx={{ textTransform: 'none', py: '6px' }}
          onMouseDown={event => {
            event.preventDefault()
            toggleMark(editor, format, constituentValue)
          }}
        >
          {content}
        </ToggleButton>
  ...
  return (
    <ToggleButtonGroup
      size='small'
      sx={{ display: 'flex', justifyContent: 'center', gap: '6px', pb: '15px' }}
    >
      <MarkButton
        format='keyword'
        keybind='(Ctrl + K)'
      ...
      <Box display='flex' flexDirection='row'>
        <MarkButton
          ...
          content={
            !constituentValue
              ? 'Alapelemek keresése:'
              : `Alapelem beszúrása: ${constituentValue}`
          }
        />
        <TextField
          size='small'
  ...
  useEffect(() => {
    const resetEditable = () => {
      ...
      for (let i = 0; i < totalNodes - 1; i++)
        Transforms.removeNodes(editor, { at: [i] }) // Remove every node except the last one, otherwise SlateJS will return error as there's no content
      for (const value of values.story) {
    ...
    <Box sx={{ width: '100%' }}>
      ...
      >
        ...
        <Editable
          ...
          onBlur={() =>
            setFieldValue(
              'automaticConstituents',
              findAutomaticConstituentsInLeaves(values.story[0].children)
            )
          }
          onKeyDown={event => {
            ...
            const previousCharacter =
              textboxContent && textboxContent[previousCharPosition]
                ? textboxContent[previousCharPosition]
                : null
            if (event.key === '"') {
              event.preventDefault()
              previousCharacter === ' ' || textboxContent === '﻿'
                ? editor.insertText('„')
                : editor.insertText('”') // "Empty" textboxes have a no-width whitespace character in them.
            }
            if (event.key === '-') {
              event.preventDefault()
              previousCharacter === ' '
