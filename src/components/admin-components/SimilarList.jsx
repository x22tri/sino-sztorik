...
const SimilarList = ({ type }) => {
    ...
    useEffect(() => {
        const fetchSimilars = async () => {
            const similarTable = await fetch(`${process.env.REACT_APP_BACKEND_URL}/admin/all-similars/${type}`);
    ...
    const handleSearch = async (event, similarGroup) => {
        ...
        if (event.key === 'Enter') {
            ...
            try {
                const searchedCharInfo = await fetch(`${process.env.REACT_APP_BACKEND_URL}/force-search/${searchTermSimilar}`)
                ...
                if (Array.isArray(searchedCharInfoJson) && searchedCharInfoJson.length === 1) {
                    ...
                    setCharacterGridRows(prev => ({
                        ...prev,
                        [similarGroup]: [...prev[similarGroup], {
                            similarGroup: similarGroup, similarType: 'appearance', 
                            charChinese: foundChar?.charChinese, keyword: foundChar?.keyword, primitiveMeaning: foundChar?.primitiveMeaning
                        }]
                       }))
                }
                else {
    ...
    const handleDeleteChar = (similarGroup, similarChar) => {
        setCharacterGridRows(prev => ({
            ...prev,
            [similarGroup]: [...prev[similarGroup]].filter(item => item !== similarChar)
        }))
    }
    ...
    return (
    <ListWrapper title={type === 'meaning' ? 'Hasonló jelentésű karakterek' : 'Hasonló kinézetű karakterek'}>
        {Object.keys(characterGridRows).map((similarGroup, index) => (
            ...
            >
                ...
                {characterGridRows[similarGroup].map ? characterGridRows[similarGroup].map((similarChar, index) => (
                    <Paper key={index} sx={{padding: '15px', position: 'relative', backgroundColor: !similarChar.similarId && theme.palette.background.charSelectAdmin }}>
                        <Box position='absolute' fontSize='10px' left='4px' top='2px'>{similarChar.similarId}</Box>
                        <Typography variant='lessonStartPaperItemChinese'>{similarChar.charChinese}</Typography>
                        <Button size='small' 
                            onClick={() => handleDeleteChar(similarGroup, similarChar)} 
                            sx={{
                            textTransform: 'none', minWidth: 0, padding: 0,  width: '12px', lineHeight: 1.2,
                            color: theme.palette.tertiary.main,
                            position: 'absolute', fontSize: '11px', right: '2px', top: '2px'
                            }}
                        >
                        ×
                        </Button>
                        <Box color={theme.palette.primary.dark}>{similarChar.keyword || ''}</Box>
                        <Box color={theme.palette.tertiary.main}>{similarChar.primitiveMeaning || ''}</Box>
                    </Paper>
                )) : <div>{JSON.stringify(characterGridRows[similarGroup])}</div>}
                    {!searchMode &&
                    <Button size='small' onClick={() => setSearchMode(similarGroup)} sx={{
                        minWidth: '0', color: theme.palette.primary.contrastText, backgroundColor: theme.palette.primary.main
                        }}
                    >
                        +
                    </Button>}
                ...
                {searchMode === similarGroup && <TextField 
