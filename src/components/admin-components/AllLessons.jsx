...
const AllLessons = () => {
  ...
  const backgroundColorInactive =
    theme.palette.type === 'dark'
      ? darken(theme.palette.grey[900], 0.1)
    ...
    useEffect(() => {
      const fetchAllLessons = async () => {
        ...
        const lessonTable = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/admin/all-lessons`
        )
        ...
        >
          {lessonState.map((lesson, index) => (
            <Grid
              ...
              key={index}
              component={Link}
              to={`/admin/lesson/${lesson.lessonNumber}`}
              sx={{
                flexBasis: { xs: '280px' },
                margin: '5px',
                padding: '10px',
            ...
            >
              ...
              {lesson.tiers.map((version, index) => (
                <Paper
                  elevation={version?.characters?.length ? 10 : 0}
                  key={index}
                  sx={{
                    textAlign: 'center',
                    position: 'relative',
                    padding: '10px',
                    my: '5px',
                    backgroundColor: version?.characters?.length
                      ? backgroundColorActive
                      : backgroundColorInactive,
                    border:
                      !version?.characters?.length &&
                      `1px dashed ${theme.palette.primary.lowPrioText}`,
                  }}
                >
                  <Box
                    ...
                    textAlign='justify'
                  >
                    {version.preface?.length > 50
                      ? version.preface?.slice(0, 70) + '...'
                      : version.preface}
                  </Box>
