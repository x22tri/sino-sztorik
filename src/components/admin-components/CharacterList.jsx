...
const CharacterList = () => {
    ...
    const characterGridColumns = [
        ...
        { field: 'charChinese', headerName: 'Írásjel', width: 70, renderCell: 
            (params) => params.row.character.charChinese
        },
        { field: 'keyword', headerName: 'Kulcsszó', width: 150, renderCell: 
            (params) => <Box color={theme.palette.primary.dark}>{params.row.character.keyword}</Box>
        },
        { field: 'primitiveMeaning', headerName: 'Alapelemként', width: 150, renderCell: 
            (params) => <Box color={theme.palette.tertiary.main}>{params.row.character.primitiveMeaning}</Box>
        },
        { field: 'story', headerName: 'Történet', width: 300, renderCell: 
            (params) => params.row.character.story
        },
        { field: 'frequency', headerName: 'Gyakoriság', width: 100, renderCell: 
            (params) => (
                params.row.character.frequency && (
                <Box margin='auto'>
                    <FontAwesomeIcon icon={faChartBar} size='xl' color={frequencyCheck(params.row.character.frequency)[0]} />
                </Box>
            ))
        },
        { field: 'pinyin', headerName: 'Kiejtés', width: 70, renderCell: 
            (params) => params.row.character.pinyin
        },
    ...
    useEffect(() => {
        let lesson
        if (lessonState && +lessonId) lesson = lessonState.find(lessonObject => lessonObject.lessonNumber === +lessonId)
        
        let allCharactersArray = []
        if (lesson?.tiers) {
            for (let i = 0; i < lesson.tiers.length; i++) {
                if (lesson.tiers[i]?.characters) {
                    allCharactersArray = [...allCharactersArray, ...lesson.tiers[i].characters]
                }
            }
    ...
    <ListWrapper title={`${lessonId}. lecke karakterei`}>
        <DataGrid
            ...
            initialState={{ sorting: { sortModel: [{ field: 'indexInLesson', sort: 'asc' }] } }}
            getRowClassName={(params) => `data-grid-tier-bg--${params.row.tier}`}
            disableSelectionOnClick
            onCellClick={(_, event) => {event.defaultMuiPrevented = true}}
            onRowClick={(row) => navigate(`/admin/character/${row.row.charId}`, {state: { row: row.row }})}
        />
    </ListWrapper>            
)}
