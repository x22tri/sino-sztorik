  ...
  return (
    <>
      <Box className='heading-box'>
        <Typography variant='h4' color={theme.palette.primary.lowPrioText}>
          {text}
        </Typography>
        {headingBoxEndText && (
          <Typography variant='h4' color={headingBoxEndTextColor}>
            {headingBoxEndText}
          </Typography>
        )}
      </Box>
...
const ArrowBox = ({ characterArray, currentCharacterIndex, onArrowClick }) => {
  const previousChar = characterArray[currentCharacterIndex - 1]
  const nextChar = characterArray[currentCharacterIndex + 1]
  ...
  return (
    <Box className='arrows-box'>
      <ArrowButton
        ...
        text={
          (previousChar?.keyword &&
            `${previousChar.keyword} (${previousChar.charChinese})`) ||
          (previousChar?.primitiveMeaning &&
            `${previousChar.primitiveMeaning} (${previousChar.charChinese})`) ||
      ...
      <ArrowButton
        ...
        text={
          (nextChar?.keyword &&
            `${nextChar.keyword} (${nextChar.charChinese})`) ||
          (nextChar?.primitiveMeaning &&
            `${nextChar.primitiveMeaning} (${nextChar.charChinese})`) ||
          'Lecke befejezése'
        }
      />
    </Box>
  ...
  if (!currentChar) return null
    ...
    return (
      <Box className='character-card-box'>
        <Paper
          className='character-paper'
          {...{ elevation }}
          sx={{ border: paperBorder }}
        >
          <HeadingBox
            text='KULCSSZÓ'
          ...
          />
          <CharacterInfo char={currentChar} />

          <HeadingBox
            text='TÖRTÉNET'
            headingBoxEndText={
              currentChar.prequel ? `ELŐZMÉNY: ${currentChar.prequel}` : null
            }
          />
          <Story story={currentChar.story} {...{ dispatch, state }} />
        </Paper>

        {(illustration || currentChar.notes) && (
          <Paper
            variant='outlined'
            className='illustration-notes-paper'
            sx={{ backgroundColor: theme.palette.background.taglineSecondary }}
          >
            {illustration && (
              <>
                ...
                <Box className='illustration'>
                  <img
                    src={illustration}
                    alt={
                      currentChar.illustrationAltText ||
                      'A karakter illusztrációja.'
                    }
                  />
                </Box>
              </>
        ...
        {characterArray && onArrowClick && (
          <ArrowBox
            {...{ characterArray, currentCharacterIndex, onArrowClick }}
          />
        )}
      </Box>
    )
