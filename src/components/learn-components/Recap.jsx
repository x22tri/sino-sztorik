...
const Recap = ({ anchorEl, content, setAnchorEl, setClickedIndex, dispatch }) => {
    ...
    const recapFieldRef = (ref) => {if (ref) setFieldRef(ref)}
    useEffect(() => {
        if (fieldRef) fieldRef.scrollIntoView({behavior: "smooth", block: "nearest"})
    }, [fieldRef, value]);
    ...
    useEventListener('keydown', ({key}) => {
        ...
        } else if (key !== 't') { // Toggling theme shouldn't close the popper.
            handleClose() 
        }
    }, true)
    ...
    if (!characters) return null
        ...
        <Popper placement='bottom-start' open={!!anchorEl} {...{anchorEl}} style={{zIndex: 1050}}
        modifiers={[{name: 'flip', enabled: false}, {name: 'preventOverflow', enabled: false}]}>
            <ClickAwayListener onClickAway={handleClose}>
                <Box display='flex' flexDirection='column' width='600px' sx={{
                ...
                >
                    ...
                    <Box sx={{ borderBottom: `1px solid ${theme.palette.divider}` }}>
                        <Tabs textColor="inherit" onChange={handleTabChange} {...{value}} centered variant='fullWidth'
                        TabIndicatorProps={{sx: {height: 3, backgroundColor: theme.palette.primary.dark} }}>
                            {characters.map((char, index) => (
                                <Tab key={index} component={forwardRef((props, ref) => (
                    ...
                    <Box ref={recapFieldRef}>
                        ...
                        <TabPanel key={index} {...{value, index}}>
                            {char?.character?.story ? <Story small story={char.character.story} /> : 'A történet nem elérhető.'}
                        </TabPanel>
                        ))}
                    </Box>
                </Box>
