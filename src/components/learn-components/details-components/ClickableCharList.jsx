...
const ClickableCharList = ({ in: inProp, nameAndIcon, sourceArray, state, dispatch, transitionTimeout, type }) => {    
    ...
    useEffect(() => {
        ...
        storyRefs.current = sourceArray.map((_, i) => storyRefs.current[i] ?? createRef())
    ...
    <>
        <ListHeading
            badgeContent={(type !== 'constituentList' && !inProp) ? sourceArray.length : 0}
        ...
        <ConditionalWrapper
            ...
            wrapper={children => <Collapse in={inProp} timeout={transitionTimeout}>{children}</Collapse>}
        >
            ...
            {type !== 'phrases' &&
                ...
                const [ mainCharacterName, secondaryCharacterName ] = [
                    characterNameFinder(character.character)[0],
                    characterNameFinder(character.character)[1]
                ]
                
                return (<ClickableCharListElement
                    key={index}
                    id={character.character?.charChinese || null}
                    ref={storyRefs.current[index]}
                    fade={type === 'constituentList'}
                    in={inProp}
                    
                    firstElement={character.character?.charChinese}
                    secondElement={mainCharacterName}
                    thirdElement={secondaryCharacterName}
                    {...{character, dispatch, index, state, transitionTimeout}}
                />)})}
            {type === 'phrases' && 
