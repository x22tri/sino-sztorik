...
const ChineseChar = ({ char, small, unknownElement }) => {
    ...
    return (
    <Box className={`chinese-char-wrapper-${small ? 'small' : 'big'}`} sx={{background: borderColors[keywordPrimitiveStatusFinder(char)] }}>
        <Box className={`chinese-char-inner-${small ? 'small' : 'big'}`} backgroundColor={theme.palette.background.paper}>
            <Box display='table-row' marginTop='10px'>
                ...
                <PopupTooltip title={'Gyakran az ezt a karaktert tartalmazó más karaktereket is így, vagy hasonlóan kell majd kiejteni.'}>
                    <Box className='pinyin-productive' color={theme.palette.primary.lowPrioText}>
                        <PodcastsIcon />
                    </Box>
                </PopupTooltip>}
                <Typography variant={small ? 'recapPinyin' : 'pinyin'} sx={{textTransform: 'none'}}>
                {char?.pinyin || unknownElement.pinyin}
                </Typography>
            </Box>
            <Box display='table-row'>
                <Typography variant={small ? 'recapChinese' : 'h3'} 
                    sx={{ lineHeight: char?.pinyin ? 0.8 : 1.2, paddingBottom: char?.pinyin ? '14px' : 0 }}
                >
    ...
    return (
        <Box display='table-row'>
            ...
            <Box className='charinfo-primitive-box' sx={{ marginLeft: tableMargin }}>
                <KeyboardReturnIcon sx={{fontSize: small ? 14 : 28, transform: 'scaleX(-1)'}}/>
                <Box component='span' fontSize={small ? 12 : 20} paddingRight={small ? '2px' : '6px'}>
                    <FontAwesomeIcon icon={faCube} color={theme.palette.tertiary.main} />
                </Box>
                <Box component='span'>
                    <Typography variant={small ? 'recapPrimitive' : 'primitiveMeaning'} color={theme.palette.tertiary.main} sx={{textTransform: 'none'}}>
...
const PrimitiveOnly = ({ char, small }) => {
    ...
    return (
        <>
        <Box component='span' sx={{fontSize: small ? 16 : 40, paddingRight: small ? '4px' : '10px' }}>
            <FontAwesomeIcon icon={faCube} color={theme.palette.tertiary.main} />
        </Box>
        {char.primitiveMeaning}
        </>
...
const CharacterInfo = ({ char, small }) => {
    ...
    const [renderedFirstElement, renderedSecondElement, firstElementColor] = keywordPrimitiveStatusDictionary[keywordPrimitiveStatusFinder(char)]
    ...
    <>
        <Box display='table-row'>
            ...
            <Box display={small ? 'block' : 'table-cell'} sx={{verticalAlign: 'bottom', paddingBottom: '4px'}}>

                {/* If the character has an "explanation", render the keyword with a tooltip. */}
                <ConditionalWrapper
                    condition={renderExplanationCondition}
                    wrapper={children => 
                    <PopupTooltip title={char.explanation}>
                    {children}
                    </PopupTooltip>}
                >
                    <Typography 
                    ...
                    sx={{
                        ...
                        textTransform: 'none',
                        textDecoration: renderExplanationCondition && `underline dashed 1px`,
