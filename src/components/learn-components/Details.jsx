...
import { faEye, faLightbulb, faCubes, faBook, faChartBar, faCommentDots } from "@fortawesome/free-solid-svg-icons";
...
const nameIconDictionary = {
    ...
    frequency: ['GYAKORISÁG', faChartBar],
    otherUses: ['EGYÉB JELENTÉSEK', faCommentDots]
}

export default function Details({ currentChar, detailsToggle, dispatch, state, wideDetails }) {
    ...
    if (!currentChar) return null
        ...
        <List dense className='details-box centerer'
        sx={{display: { xs: 'none', md: 'flex' }, backgroundColor: theme.palette.background.default, paddingTop: '6px'}}
        >
            <Collapse 
            ...
            sx={{ '& .MuiCollapse-wrapperInner': { width: `${detailsWidthExpanded}px` } }}
            >
                    {/* Expand/Collapse chevron button */}
                    {(currentChar.character?.constituents?.length || currentChar.character?.frequency) &&
                    <PopupTooltip title={'Részletek megjelenítése/elrejtése (R)'}>
                        <Box className='chevron-box' style={{transition: `transform ${transitionTimeout}ms`, transform: !wideDetails ? '' : 'rotate(180deg)'}}>
