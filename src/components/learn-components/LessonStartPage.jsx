      ...
      <Box className='lesson-start-body'>
        <Box sx={{ color: theme.palette.secondary.main }}>
          <Typography variant='h5'>
            {`${getRomanNumeralForTier(
              currentTier
            )}. KÖR, ${currentLesson}. LECKE`}
          </Typography>
        </Box>
        <Box
          className='lesson-start-name'
        ...
        <Box
          className='lesson-start-character-list-header'
          sx={{ color: theme.palette.secondary.main }}
        >
          A lecke tartalma:
        </Box>
        ...
        <Box
          className='lesson-start-character-list'
          sx={{ color: theme.palette.primary.contrastText }}
        >
          {characterArray.map((char, index) => (
            <LessonContentCharacter key={index} char={char.charChinese} />
          ))}
        </Box>
