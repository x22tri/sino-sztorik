...
import { LowEmphasisOnColoredBGButton, SecondaryColorButton } from "../component-library/GenericComponents";

const FAQAccordion = ({question, answer}) => {
    const theme = useTheme()
    return (
    <Accordion disableGutters elevation={0} sx={{marginTop: '20px', borderRadius: '4px', '&:before': { display: 'none' } }}>
            <AccordionSummary sx={{py: '2px', flexDirection: 'row-reverse', fontWeight: 'bold',
                backgroundColor: theme.palette.background.default, 
                '& .MuiAccordionSummary-expandIconWrapper.Mui-expanded': {transform: 'rotate(90deg)'},
                '& .MuiAccordionSummary-content': {marginLeft: '5px'}
                }} expandIcon={<ArrowForwardIosSharpIcon sx={{ fontSize: '0.7rem', marginTop: '1px' }} />}
            >
            {question}
            </AccordionSummary>
            <AccordionDetails sx={{backgroundColor: theme.palette.background.default, borderTop: `1px solid ${theme.palette.divider}`,
                borderRadius: '0 0 4px 4px', textAlign: 'justify',  paddingTop: '20px'}}
            >
                {answer}
            </AccordionDetails>
        </Accordion>
...
function MainPage({themeToggle}) {
    ...
    const LoginButton = () => <LowEmphasisOnColoredBGButton onClick={() => openAuthModal('login')} text='Már van fiókom' />

    const RegisterButton = () => <SecondaryColorButton onClick={() => openAuthModal('signup')} text='Új fiók' />
    ...
    return (
        ...
        <Box component='main'>
            <Grid container component='section' spacing={10} sx={{marginTop: 0, paddingBottom: '80px', 
                    ...
                    <Grid container direction='column' display='flex'>
                        <Grid item component='header' sx={{marginBottom: '30px'}}>
                            <Box display='flex' justifyContent='center' textAlign='center'>
                                <Typography fontWeight='bold' fontSize={28} lineHeight='1.3' component='h2'>
                                Tanulj úgy kínaiul, mint egy
                                </Typography>
                            </Box>
                            <Box display='flex' justifyContent='center' textAlign='center'>
                                <Typography variant='mainPagePrimaryHeading' color={theme.palette.secondary.main} component='h1'>
                                {notSmallScreen ? 'memóriabajnok' : 'memória-bajnok'}
                                </Typography>
                            </Box>
                            </Grid>
                            <Grid item display='flex' textAlign='justify' justifyContent='center'>
                                <Box flexBasis='800px' margin='0 20px'>
                                    ...
                                    <Box display='flex' justifyContent='center' marginTop='50px' gap={3}>
                                        <RegisterButton />
                                        <LoginButton />
                                    </Box>
                                </Box>
            ...
            <Grid container component='section' direction='row-reverse' spacing={10} marginTop={0}>
                <Grid item xs={12}>
                    <Grid container direction='column' display='flex'>
                        <Grid item component='header' marginBottom='30px'>
                            <Box display='flex' justifyContent='center' textAlign='center'>
                                <Typography variant='mainPageSecondaryHeading' color={theme.palette.primary.main} component='h2'>
                                Hogyan működik?
                                </Typography>
                            </Box>
                        </Grid>
                        <Grid item display='flex' textAlign='justify' justifyContent='center'>
                                <Box flexBasis='800px' margin='0 20px 50px'>
                                    ...
                                    A kínai karakterek egy része <strong>képjel</strong>, tehát azt ábrázolja, amit jelent.
                                    Ilyen például a 口 (kŏu), ami <strong>„szájat”</strong> jelent, és úgy is néz ki, mint egy tágra nyílt száj.
                                    </ExplanatoryParagraph>

                                    <ExplanatoryParagraph>
                                    De a <strong>legtöbb karakter nem egyszerű képjel</strong>, hanem képjelek és egyéb alapelemek <strong>kombinációja. </strong>
                                    A 战 (zhàn) karaktert nézegethetjük órákig, mégsem jövünk rá, hogy jelentése <strong>„háború”</strong>.
                                    </ExplanatoryParagraph>

                                    <ExplanatoryParagraph>
                                    Mi alkotóelemeire bontottuk a <strong>3000 legfontosabb egyszerűsített kínai karaktert</strong>, és olyan
                                    <strong> sorrendbe</strong> tettük neked, hogy ne kelljen olyan karaktert megtanulnod, amelynek az alkotóelemeit nem ismered.
                                    </ExplanatoryParagraph>

                                    <ExplanatoryParagraph>
                                    És hogy ne kelljen több százszor leírnod ezeket a karaktereket, amíg meg nem jegyzed őket, <strong>történeteket írtunk </strong> hozzájuk.
                                    Ha <strong>élénken vizualizálod</strong> őket, egy életre veled maradhatnak.
                                    </ExplanatoryParagraph>
                                </Box>
                            </Grid>
                    </Grid>
        ...
        <Box display='flex' flexDirection='column' component='section' justifyContent='center' sx={{
            backgroundColor: theme.palette.type === 'dark' ? theme.palette.grey[900] : theme.palette.grey[200], color: theme.palette.tertiary.light}}
        >
            <Box component='header' margin='60px auto 30px' textAlign='center'>
                <Typography variant='mainPageSecondaryHeading' component='h2'>
        ...
        <Box display='flex' component='footer' justifyContent='center' sx={{py: '30px'}}
            backgroundColor={theme.palette.type === 'light' && theme.palette.grey[300]} 
            color={theme.palette.type === 'light' ? theme.palette.grey[500] : theme.palette.grey[800]}>
            © 2022 Dió Dávid
        </Box>
        <AuthPage {...{signupOrLoginState, setSignupOrLoginState, closeAuthModal}} />
        </>
    )
}
