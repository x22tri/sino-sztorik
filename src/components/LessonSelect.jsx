...
const TierStatus = ({ tier, status, cardInactive }) => {
  ...
  const colorDictionary = {
    'Még nincs feloldva': {
      dark: [theme.palette.grey[600], theme.palette.primary.contrastText],
      light: [theme.palette.grey[600], theme.palette.primary.contrastText],
    },
    'Soron következő lecke': {
      dark: [undefined, theme.palette.secondary.main],
      light: [undefined, theme.palette.secondary.main],
    },
    'Már megtanult lecke': {
      dark: [undefined, theme.palette.background.default],
      light: [undefined, theme.palette.primary.light],
    },
    'Ebben a körben nincs ilyen lecke': {
      dark: [theme.palette.grey[800], theme.palette.grey[700]],
      light: [theme.palette.grey[500], darken(theme.palette.primary.dark, 0.3)],
    },
  }
  ...
  return (
    <PopupTooltip title={status || 'A lecke állapotát nem sikerült lekérni.'}>
      <Box component='span' sx={{ margin: '0 5px' }}>
        {colorDictionary[status] &&
        colorDictionary[status][theme.palette.type] &&
        colorDictionary[status][theme.palette.type][+!cardInactive] ? (
          <Typography
            variant='lessonSelectTier'
            color={colorDictionary[status][theme.palette.type][+!cardInactive]}
          >
...
const LessonCard = ({ lesson, currentTier, currentLesson }) => {
  ...
  const cardInactive = !!(
    lesson.tiers[0].status === 'Még nincs feloldva' ||
    lesson.tiers[0].status === 'Ebben a körben nincs ilyen lecke'
  )
  ...
  const colorDictionary = {
    ...
    backgroundHover: {
      dark: lighten(theme.palette.grey[800], 0.1),
      light: darken(theme.palette.primary.dark, 0.1),
    },
  }

  const themeType = theme.palette.type

  const cardTextColor = cardInactive
    ? colorDictionary.textInactive[themeType]
    : colorDictionary.textActive[themeType]
  const cardBackgroundColor = cardInactive
    ? colorDictionary.backgroundInactive[themeType]
    : colorDictionary.backgroundActive[themeType]

  return (
    <Grid
      item
      position='relative'
      sx={{ flexBasis: { xs: '250px' }, margin: '20px' }}
    >
      <Paper
        className='lesson-card-parent'
        elevation={10}
        sx={{
          ...
          ':hover': {
            backgroundColor:
              !cardInactive && colorDictionary.backgroundHover[themeType],
          },
        }}
      >
        <Box
        ...
        >
          <Typography color={cardTextColor}>{lesson.lessonNumber}.</Typography>
        </Box>
        ...
        <Box
          display='flex'
          className='lesson-card-review-button'
          gap='10px'
          position='absolute'
          left='50%'
          bottom='50%'
        ...
        >
          {!cardInactive &&
            !(
              ...
              <Link to={`/review/${lesson.lessonNumber}`}>
                <WhiteButton text='Ismétlés' />
              </Link>
            )}

          {lesson.lessonNumber === currentLesson && (
            <Link to='/learn'>
              <SecondaryColorButton text='Tanulás' />
            </Link>
          )}
        </Box>
        ...
        <Box
          ...
          sx={{ transform: 'translate(-50%, 0)' }}
        >
          {lesson.tiers.map((lessonTier, i) => (
            <TierStatus
              key={i}
              tier={lessonTier.tier}
              status={lessonTier.status}
              {...{ cardInactive }}
            />
          ))}
        </Box>
...
const GreetingCard = ({
  ...
  return (
    <Box
      ...
      sx={{
        backgroundColor: nextLessonBGColor,
      ...
      >
        ...
        <Link to='/learn'>
          <SecondaryColorButton
            ...
            endIcon={
              <ArrowRightAltIcon
                sx={{ transform: 'scaleX(1.3) scaleY(1.3)' }}
              />
            }
          />
        </Link>
...
const LessonCardList = memo(({ lessonSelectData, userData }) =>
  lessonSelectData.map((lesson, index) => (
    <LessonCard
      key={index}
...
const ForceSearchButton = ({ auth, lastSearch, setSearchedChar }) => {
  ...
  return (
    ...
    >
      <SecondaryColorButton
        onClick={handleForceSearchChar}
        text='Megtekintem mégis'
      />
    </Box>
  )
}
  ...
  try {
    useEffect(() => {
      ...
      const fetchLessonSelect = async () => {
        const lessonTable = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/learn/select`,
          {
            headers: { Authorization: `Bearer ${auth.token}` },
          }
        )
      ...
      <>
        ...
        <main>
              ...
              >
                <Grid item xs={12} marginTop='40px'>
                  <Typography>
                    Inkább{' '}
                    <strong>
                      át szeretnél ismételni egy már megtanult leckét
                    </strong>
                    ? Itt kiválaszthatod.
                  </Typography>
                  <Typography>
                    (Az ismétlés nincs hatással az előrehaladásodra.)
                  </Typography>
                </Grid>
          ...
          {searchedChar?.message && (
            <ErrorField>
              {searchedChar.message}
              {searchedChar.status === 401 && (
                <ForceSearchButton {...{ auth, lastSearch, setSearchedChar }} />
              )}
            </ErrorField>
          )}
        </main>
