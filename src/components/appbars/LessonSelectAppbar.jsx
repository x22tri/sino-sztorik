...
import { faUser, faTools, faCog, faSignOutAlt, faSearch } from '@fortawesome/free-solid-svg-icons'
import ArrowRightAltIcon from '@mui/icons-material/ArrowRightAlt';

import AuthContext from "../../context/auth-context";
import { AppbarWrapper, TopbarButton, ThemeChangeButton, Logo } from './AppbarWrapper'
...
const SearchField = ({setSearchedChar, setLastSearch}) => {
    ...
    const handleSearch = async (event) => {
        ...
        if (event.key === 'Enter') {
            ...
            try {
                ...
                const searchedCharInfo = await fetch(`${process.env.REACT_APP_BACKEND_URL}/search/${searchTerm}`, {
    ...
    return (
        <Box display='flex' flex={2}>     
            ...
            sx={{
            '& .MuiInputLabel-root': { color: theme.palette.primary.topbarButton, top: '-3px', fontSize: 14, fontWeight: 'bold', },
            '&:hover .MuiInputLabel-root': { color: theme.palette.secondary.main },
            ...
            '& .MuiFilledInput-root': { backgroundColor: theme.palette.type === 'dark' && theme.palette.grey[900]}
            }}/>
        </Box>
    )
}

const LessonSelectAppbar = ({themeToggle, searchedChar, setSearchedChar, setLastSearch}) => {
    ...
    const ProfileButton = () => {
        ...
        const handleLogout = () => {auth.logout(); setAnchorEl(null)}

        return (
            <>
            <TopbarButton onClick={handleClick} iconContent={<FontAwesomeIcon icon={faUser}/>} />
    ...
    const BackToLessonSelectButton = () => (
        <TopbarButton onClick={() => {setSearchedChar(null); document.title = 'Sino-sztorik'}}
        textContent='Vissza a leckeválasztóba' startIcon={<ArrowRightAltIcon sx={{transform: 'scaleX(-1.3) scaleY(1.3)'}}/>}
        />
    )
