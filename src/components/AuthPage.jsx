...
import { SecondaryColorButton } from "../component-library/GenericComponents";
...
function AuthPage({signupOrLoginState, setSignupOrLoginState, closeAuthModal}) {
    ...
    const sendAuthRequest = async (values) => {
      try {
          const response = await fetch(signupOrLoginState === 'login' ? loginRoute : signupRoute, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
            body: signupOrLoginState === 'login' ? 
                JSON.stringify({
                email: values.email,
        ...
        else auth.login(responseData.userId, responseData.token)
      } catch (err) {
        throw new Error(err)
      }
    }
    ...
    const validate = values => {
      ...
      if (values.password && !values.confirmPassword && signupOrLoginState === 'signup') {
        errors.confirmPassword = 'Kérjük, erősítsd meg jelszavad.';
      } 
      else if (values.password && values.confirmPassword !== values.password && signupOrLoginState === 'signup') {
    ...
    const AuthTextInput = ({ label, ...props }) => {   
      ...
      return (
          <TextField {...field} {...{label}} {...props} sx={{
    ...
    <Paper sx={{padding:'30px 0', textAlign: 'center'}}>
        ...
        >
          ...
          <Box display='flex' flexDirection='column' alignItems='center' marginTop='10px'>
            ...
            {signupOrLoginState === 'signup' && <AuthTextInput label="Jelszó megerősítése" name="confirmPassword" type="password" />}
            {signupOrLoginState === 'signup' && <AuthTextInput label="Hogyan szólíthatunk?" name="displayName" type="text" />}
            </Stack>
            ...
            <SecondaryColorButton type='submit' text={signupOrLoginState === 'login' ? 'Bejelentkezés' : 'Fiók létrehozása'}/>
            ...
            <>
              <Box marginTop='30px'>
                ...
                <Box component='span' sx={{cursor: 'pointer', color: theme.palette.primary.link, '&:hover': {opacity: 0.8} }} 
                onClick={() => setSignupOrLoginState('signup')}> Hozz létre új fiókot</Box>
              </Box>
            </>
            : 
            <>
              <Box marginTop='30px'>
                ...
                <Box component='span' sx={{cursor: 'pointer', color: theme.palette.primary.link, '&:hover': {opacity: 0.8} }} 
                onClick={() => setSignupOrLoginState('login')}> Jelentkezz be</Box>
              </Box>
            </>
            }
        </Box>
